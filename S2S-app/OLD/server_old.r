...
server = function(input, output, session) {
  ...
  realtime <- reactiveFileReader(100000, session, 'realtime/realtimeCFS_anom.rds', LoadToEnvironment)

  ## for ui real time tab
  output$ui_fcst_dates = renderUI({
  ...
  output$realtime_output = renderLeaflet({
    ...
    var_val = filter(realtime(), 
                     var_name == input$var_real_sel, 
                     lead == input$lead_real_sel, 
  ...
  output$biwk_analysis = renderLeaflet({
    ...
    var_val = filter(t.p_stats, 
                     var == input$var_sel, 
                     leads == input$lead_sel, 
                     stats == input$stat_sel,
                     season == input$season_sel)$value
    
    ## add variable to shape file data frame
    huc4@data$var = var_val

    ## change legend title
    if(input$stat_sel == 'acc') {
      leg_title = 'Anomaly Correlation'
    } else if (input$stat_sel == 'meanErr' & input$var_sel == 'tmp2m') {
      leg_title = 'Bias (&deg;C)'
    } else if (input$stat_sel == 'meanErr' & input$var_sel == 'prate') {
      leg_title = 'Bias (mm/d)'
    } else {
    ...
    if(input$stat_sel == 'acc') {
      ...
      pal = colorBin(col_vec,
                     ...
                     reverse = T, na.color = "#808080", alpha = FALSE)
      # Bias
      } else if (input$stat_sel == 'meanErr') {
  ...
  output$analysis <- renderText({  
    includeMarkdown("bi_week_analysis.md")  
  })
