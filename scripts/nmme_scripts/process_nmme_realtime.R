...
## Directories
dir_in = '/home/sabaker/s2s/nmme/files/HUC_fcst/'
...
for (i in 1:nrow(file.names)) {
  ...
  df_model = rbind.fill(df_model, df)
  
}
colnames(df_model) <- c('var', 'mdl', 'fcst_date', 'hru', 'lead 0', 'lead 1', 'lead 2', 'lead 3', 'lead 4', 'lead 5', 'lead 6', 'lead 7')
...
## organize
colnames(realtime) <- c('var', 'mdl', 'fcst_date', 'hru', '0', '1', '2', '3', '4', '5', '6', '7')
...
real_3mon = real_3mon %>% group_by(var, mdl, fcst_date, hru) %>% summarise_each(funs(mean))
...
## convert units - mm/s to in/wk & C to F
